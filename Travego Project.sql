...
select passenger.passenger_name, passenger.boarding_city, passenger.destination_city, passenger.Bus_type, Price.price
 from passenger inner join price on passenger.bus_type = price.bus_type where (price.distance = passenger.distance);

# e. What are the passenger name(s) and the ticket price for those who traveled 1000KMs Sitting in a bus?
select passenger.passenger_name, passenger.boarding_city, passenger.destination_city, passenger.Bus_type, passenger.distance, Price.price
 from passenger inner join price on passenger.bus_type = price.bus_type 
 where (price.distance = passenger.distance and passenger.distance = 1000 and passenger.bus_type = 'sitting');

# f. What will be the Sitting and Sleeper bus charge for Pallavi to travel from Bangalore to Panaji?
select bus_type,price from price where (distance = (select distance from passenger where boarding_city in ('Panaji','Bengaluru') 
and Destination_city in ('Bengaluru','Panaji'))) group by bus_type;
